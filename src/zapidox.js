...
(function(global, factory) {
    ...
    Zapidox.generate = function(dataset_url, example){

        let options = {bootstrap4: true, example: example};

        let parsed = parseErddapUrl(dataset_url);
        let erddap = new ErddapClient(parsed.erddap_url);

        return generateAPIDocs(erddap,parsed.dataset_id,options).then(function(apidocs){
    ...
    var generateMethodDocs = function(dataset, method, options, toc, dataset_link) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var generateFormatMethodDocs = function(format) {
                ...
                if (options.tableInTitle) {
                    ...
                    if (options.bootstrap4) {
                        ...
                        if (toc.indexOf(title) < 0) {
                            ...
                            output.push('<hr />');
                            output.push("<div class='tab-pane'>");
                            output.push('<ul class="nav nav-pills">');
                            output.push('<li class="nav-item"><a class="nav-link active show" data-toggle="pill" href="#' + id_prefix + '">' + formatNoExtension + '</a></li>');
                        ...
                        output.push('<div id="' + id_prefix + '" class="tab-pane ' + (formatSelected ? "active show" : "fade") + '">');
                ...
                var getBeforeCode = function(lang) {
                    return "\n```" + lang
                };
                var getAfterCode = function() {
                    return "```"
                };
                var output_end = "";
                ...
                if (options.bootstrap4) {
                    ...
                    getQueryOutput(partial_url, params).then(queryOutput => {
                        console.log("todo write output",queryOutput);
                        return;
                        document.getElementById(output_id).innerHTML= options.format(queryResultToMarkdown(queryOutput));
                    }, function(e) {
    ...
    var getDatasetLink = function(erddap_url, dataset_id, dataset_type) {
    ...
    var getOverview = function(ncglobal, dataset_link, options, dataset_type) {
        var headline = [
    ...
    var getVariablesTable = function(meta, _type, dataset_id, options, toc, dataset_link) {
        let [singular,plural] = {"variable":["Variable","Variables"],"dimension":["Dimension","Dimensions"]}[_type];
        var rows = [
